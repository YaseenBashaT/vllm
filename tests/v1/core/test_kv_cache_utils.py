...
def test_validate_extensible_kv_cache_leaves_supported_config_alone(monkeypatch):
    # The platform gate is part of this validator; pin a supported platform
    # so the test result does not depend on the machine running it.
    # Patch the class (as the off-CUDA tests do): patching the shared
    # instance would leave a shadowing instance attribute behind on undo.
    monkeypatch.setattr(
        type(kv_cache_utils.current_platform), "is_cuda_alike", lambda self: True
    )
